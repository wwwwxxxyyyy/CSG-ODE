...
def compute_masked_likelihood(mu, data, mask, likelihood_func,temporal_weights=None):
	...
	if temporal_weights!= None:
		...
		log_prob_masked = torch.sum(log_prob * mask * weight_for_times, dim=2)  
	else:
		log_prob_masked = torch.sum(log_prob * mask, dim=2)  


	timelength_per_nodes = torch.sum(mask.permute(0,1,3,2),dim=3)
	assert (not torch.isnan(timelength_per_nodes).any())
	log_prob_masked_normalized = torch.div(log_prob_masked , timelength_per_nodes)
	res = torch.mean(log_prob_masked_normalized, -1) 
