...
class VAE_Baseline(nn.Module):
	...
	def get_gaussian_likelihood(self, truth, pred_y,temporal_weights, mask ):
		...
		log_density_data = masked_gaussian_log_density(pred_y, truth_repeated,
			obsrv_std = self.obsrv_std, mask = mask,temporal_weights= temporal_weights) 
	...
	def compute_all_losses(self, batch_dict_feature_encoder,
								 ...
								 batch_dict_decoder,batch_dict_graph ,n_traj_samples = 1, kl_coef = 1.):
	
		pred_y, info,temporal_weights= self.get_reconstruction(batch_dict_feature_encoder,
															   batch_dict_edges_importance_encoder,
                          								       batch_dict_observed_mask_encoder,
                         							           batch_dict_avg_interval_encoder,
                                 							   batch_dict_time_pos_encoder,
